...
.PHONY: run
run: $(DISK_IMG_PATH)
	qemu-system-x86_64 -drive if=pflash,file=OVMF_CODE.fd -drive if=pflash,file=OVMF_VARS.fd -hda $(DISK_IMG_PATH)
...
# $(KERNEL_PATH): ./kernel/src/*.rs ./kernel/Cargo.toml ./kernel/.cargo/* ./kernel/x86_64-unknown-none-mikanos-rust.json
